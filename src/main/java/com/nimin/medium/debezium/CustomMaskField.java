package com.nimin.medium.debezium;
...
public abstract class CustomMaskField<R extends ConnectRecord<R>> implements Transformation<R> {
    ...
    private R applyWithSchema(R record) {
        ...
        for (Field field : value.schema().fields()) {
            if(field.schema().type().equals(STRUCT) && ! field.schema().fields().isEmpty()){
                final Struct newValue = new Struct(field.schema());
                final Struct fieldValues = value.getStruct(field.name());
                if(fieldValues == null){
                    updatedValue.put(field,value.get(field));
                }else {
                    for (Field newField : field.schema().fields()) {
                        final Object newFieldValue = fieldValues.get(newField);
                        newValue.put(newField, maskedFields.contains(newField.name()) ? masked(newFieldValue) : newFieldValue);
                    }
                    updatedValue.put(field, newValue);
                }
            }else {
    ...
    public static final class Key<R extends ConnectRecord<R>> extends CustomMaskField<R> {
        @Override
    ...
    public static final class Value<R extends ConnectRecord<R>> extends CustomMaskField<R> {
        @Override
        protected Schema operatingSchema(R record) {
